## Simplified DataFile implementation for immediate writes
##
## This is a simplified version that doesn't use complex buffering.
## The full buffered version will be implemented later.
...
proc readHeader*(df: var DataFile): FileHeader =
  ## Read the file header (thread-safe)
  withLock(df.lock):
...
proc appendRecord*(df: var DataFile, key: string, value: string, timestamp: int64): RecordInfo =
  ...
  withLock(df.lock):
    ...
    if df.shouldFsync and when defined(posix):
...
# Export the simplified DataFile for now
export writebuffer
# export writebuffer